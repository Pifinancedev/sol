...
contract USDTDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() public DividendPayingToken("USDT_Dividen_Tracker", "USDT_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 5000 * (10**4) * (10**18); //must hold 50,000,000+ tokens
    }
...
contract PiFinance is ERC20, Ownable {
    ...
    uint256 public totalSupply_ = 1000 * (10**8) * (10**18);
    ...
    uint256 public USDTRewardsShare = 3;
    ...
    constructor() public ERC20("PiFinance", "PIFI") {
    ...
    function blacklistMultiAddresses(address[] calldata accounts, bool value) external onlyOwner{
        for(uint256 i = 0; i < accounts.length; i++) {
    ...
    function _transfer(
        ...
        if(_isExcludedFromFees[from] || _isExcludedFromFees[to] || isJustTransfer) {
            takeFee = false;
        }
